...
- **Single host.** All `tint.sh` traffic terminates at the Cloudflare Worker. There is no other origin.
...
- **No `dist/tint` file.** `/tint` is a code path, not a static file. A file at that path would shadow the redirect.
...
- **`/tint` redirect target preserves `download_count`.** The target is `releases/latest/download/<asset>`, which GitHub itself 302s to the active release asset. The second hop is what increments the per-asset counter.
- **Fallthrough.** All other paths delegate to the static assets binding. No path-based routing tables.
...
- **Single deploy path.** [`.github/workflows/deploy.yml`](../.github/workflows/deploy.yml) is the only writer to the Cloudflare Worker. Cloudflare's built-in Git auto-deploy stays disabled.
...
- **DNS is on Cloudflare.** Apex `tint.sh` is bound to the Worker as a custom domain (no maintained A/AAAA records).
