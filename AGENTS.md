...
- **Cloudflare Workers Builds is the only automated deployer.** Both production (master push) and per-PR previews come from Builds. There is no `.github/workflows/deploy.yml` and no `CLOUDFLARE_API_TOKEN` repo secret. The CI gate runs as the wrangler `build.command` in [`wrangler.jsonc`](wrangler.jsonc), so it executes regardless of which entrypoint invokes wrangler. Manual `wrangler deploy` / `wrangler rollback` from an authenticated checkout, and dashboard rollback/promote actions, also write to production — they are the deliberate operator escape hatches and bypass the gate by design. Adding a second _automated_ deployer (e.g., a re-introduced GitHub Action that calls `wrangler deploy`) would race Builds and is forbidden.
